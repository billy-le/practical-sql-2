...
FROM
    'C:\YourDirectory\pls_fy2018_libraries.csv' WITH (FORMAT CSV, HEADER);
...
FROM
    'C:\YourDirectory\pls_fy2017_libraries.csv' WITH (FORMAT CSV, HEADER);

COPY pls_fy2016_libraries
FROM
    'C:\YourDirectory\pls_fy2016_libraries.csv' WITH (FORMAT CSV, HEADER);
...
SELECT
    ...
    sum(pls16.visits) AS visits_2016,
    round(
        (sum(pls18.visits::numeric) - sum(pls17.visits)) / sum(pls17.visits) * 100,
        1
    ) AS chg_2018_17,
    round(
        (sum(pls17.visits::numeric) - sum(pls16.visits)) / sum(pls16.visits) * 100,
        1
    ) AS chg_2017_16
FROM
    pls_fy2018_libraries pls18
...
SELECT
    ...
    round(
        (
            sum(pls18.totstaff::numeric) - sum(pls17.totstaff)
        ) / sum(pls17.totstaff) * 100,
        1
    ) AS chg_staff_2018_17,
    round(
        (
            sum(pls17.totstaff::numeric) - sum(pls16.totstaff)
        ) / sum(pls16.totstaff) * 100,
        1
    ) AS chg_staff_2017_16
FROM
    pls_fy2018_libraries pls18
    JOIN pls_fy2017_libraries pls17 ON pls18.fscskey = pls17.fscskey
    JOIN pls_fy2016_libraries pls16 ON pls18.fscskey = pls16.fscskey
