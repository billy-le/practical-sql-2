...
FROM
    farmers_markets
ORDER BY
    geog_point <-> ST_GeogFromText('POINT(-68.2041607 44.3876414)')
LIMIT
    3;
...
FROM
    ...
    JOIN us_counties_pop_est_2019 c ON sh.statefp = c.state_fips
...
-- Import (for use on command line if on macOS or Linux; see Chapter 18)
shp2pgsql - I - s 4269 - W LATIN1 tl_2019_35049_linearwater.shp santafe_linearwater_2019 | psql - d analysis - U postgres shp2pgsql - I - s 4269 - W LATIN1 tl_2019_35049_roads.shp santafe_roads_2019 | psql - d analysis - U postgres
...
SELECT
    statefp AS st,
    round(
        (sum(ST_Area(geom::geography)))::numeric,
        2
    ) AS sqm
FROM
    us_counties_2019_shp
...
distance AS (
    SELECT
        ST_Distance(oak.geog_point, columbia.geog_point)
    FROM
        oak,
        columbia
)
