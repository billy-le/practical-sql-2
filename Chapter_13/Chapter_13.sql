...
SELECT
    ...
    round(
        (
            est.establishment_count / census.pop_est_2018::numeric
        ) * 1000,
        1
...
FROM
    ...
    LATERAL (
        SELECT
            pop_est_2019 - pop_est_2018 AS raw_chg
    ) rc,
    LATERAL (
        SELECT
            raw_chg / pop_est_2018::numeric AS pct_chg
    ) pc
ORDER BY
    pct_chg DESC;
...
INSERT INTO
    teachers_lab_access (access_time, lab_name, teacher_id)
VALUES
...
SELECT
    ...
    round(
        (
            establishments.establishment_count / counties.pop_est_2018::numeric(10, 1)
        ) * 1000,
        1
    ) AS estabs_per_thousand
FROM
...
WITH temps_collapsed (max_temperature_group) AS (
    SELECT
        CASE
            ...
            WHEN max_temp >= 88 THEN '88 - 89'
            WHEN max_temp >= 86 THEN '86 - 87'
            WHEN max_temp >= 84 THEN '84 - 85'
            WHEN max_temp >= 82 THEN '82 - 83'
            WHEN max_temp >= 80 THEN '80 - 81'
            ELSE '79 or less'
        END
    FROM
        temperature_readings
    WHERE
        station_name ILIKE 'waikiki%'
)
SELECT
    max_temperature_group,
    count(*)
