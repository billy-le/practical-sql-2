...
SELECT
    regexp_replace('05/12/2024', '\d{4}', '2023');
...
SET
    ...
    date_2 = CASE
        ...
        WHEN (
            SELECT
                regexp_match(original_text, '-(\d{1,2}\/\d{1,2}\/\d{2})') IS NULL
        )
        AND (
            SELECT
                regexp_match(original_text, '\/\d{2}\n\d{4}-(\d{4})') IS NOT NULL
        ...
        WHEN (
            SELECT
                regexp_match(original_text, '-(\d{1,2}\/\d{1,2}\/\d{2})') IS NOT NULL
        )
        AND (
            SELECT
                regexp_match(original_text, '\/\d{2}\n\d{4}-(\d{4})') IS NOT NULL
...
FROM
    'C:\YourDirectory\president_speeches.csv' WITH (FORMAT CSV, DELIMITER '|', HEADER OFF, QUOTE '@');
...
FROM
    president_speeches
WHERE
    search_speech_text @@ to_tsquery('english', 'military <2> defense')
ORDER BY
    speech_date;
...
-- Try It Yourself
SELECT
    regexp_replace('Alvarez, Jr. and Williams, Sr.', ',', '');

SELECT
    (regexp_matches('Alvarez, Jr.', ',\s*(\w+\.?)')) [1]
