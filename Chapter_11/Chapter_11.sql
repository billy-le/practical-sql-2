...
FROM
    'C:\YourDirectory\acs_2014_2018_stats.csv' WITH (FORMAT CSV, HEADER);
...
FROM
    'C:\YourDirectory\cbp_naics_72_establishments.csv' WITH (FORMAT CSV, HEADER);
...
SELECT
    ...
    pop.pop_est_2018,
    round(
        (cbp.establishments::numeric / pop.pop_est_2018) * 1000,
        1
    ) AS estabs_per_1000
FROM
...
WHERE
    pop.pop_est_2018 >= 50000
ORDER BY
    cbp.establishments::numeric / pop.pop_est_2018 DESC;
...
COPY us_exports
FROM
    'C:\YourDirectory\us_exports.csv' WITH (FORMAT CSV, HEADER);
...
-- Try It Yourself
SELECT
    corr(median_hh_income, pct_masters_higher) AS masters_corr
FROM
    acs_2014_2018_stats;
