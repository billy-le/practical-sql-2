...
FROM
    'C:\YourDirectory\MPI_Directory_by_Establishment_Name.csv' WITH (FORMAT CSV, HEADER);
...
UPDATE
    meat_poultry_egg_establishments original
SET
    st = BACKUP.st
FROM
...
COPY state_regions
FROM
    'C:\YourDirectory\state_regions.csv' WITH (FORMAT CSV, HEADER);

-- Listing 10-19: Adding and updating an inspection_deadline column
ALTER TABLE
    meat_poultry_egg_establishments
ADD
    COLUMN inspection_deadline timestamp WITH time zone;

UPDATE
    meat_poultry_egg_establishments establishments
SET
...
ALTER TABLE
    meat_poultry_egg_establishments DROP COLUMN zip_copy;
...
ALTER TABLE
    meat_poultry_egg_establishments RENAME TO meat_poultry_egg_establishments_temp;

ALTER TABLE
    meat_poultry_egg_establishments_backup RENAME TO meat_poultry_egg_establishments;

ALTER TABLE
    meat_poultry_egg_establishments_temp RENAME TO meat_poultry_egg_establishments_backup;

-- Try It Yourself
CREATE TABLE meat_poultry_egg_establishments_new AS
SELECT
    *,
    false::boolean meat_processing,
    false::boolean poultry_processing
FROM
    meat_poultry_egg_establishments;
...
SELECT
    *
FROM
    meat_poultry_egg_establishments_new;

ALTER TABLE
    meat_poultry_egg_establishments RENAME TO meat_poultry_egg_establishment_temp;

ALTER TABLE
    meat_poultry_egg_establishments_new RENAME TO meat_poultry_egg_establishments;
...
SELECT
    count(*) AS meat_poultry_processors
FROM
    meat_poultry_egg_establishments
WHERE
    meat_processing = TRUE
...
-- From ChatGPT as a single query
SELECT
    COUNT(
        CASE
            WHEN meat_processing = TRUE THEN 1
        END
    ) AS meat_processors,
    COUNT(
        CASE
            WHEN poultry_processing = TRUE THEN 1
        END
    ) AS poultry_processors,
    COUNT(
        CASE
            WHEN meat_processing = TRUE
            AND poultry_processing = TRUE THEN 1
        END
    ) AS meat_poultry_processors
FROM
    meat_poultry_egg_establishments;
